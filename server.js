...
// กำหนดเงื่อนไขการตรวจจับเฉพาะ "ชื่อบัญชีผู้รับเงิน" และ "รหัสร้านค้า/Biller ID" เท่านั้น
...
app.post('/api/verify-slip', (req, res, next) => {
    ...
    try {
        ...
        if (result && result.status === 200) {
            ...
            // --- 1. ตรวจสอบเฉพาะ "ชื่อบัญชีผู้รับเงิน" และ "รหัสร้านค้า" ---
            const fullSlipStr = JSON.stringify(slipData);
            
            const matchName = TARGET_RECEIVER_NAMES.some(name => fullSlipStr.includes(name));
            const matchCode = TARGET_SHOP_CODES.some(code => fullSlipStr.includes(code));

            // ต้องมีทั้งชื่อผู้รับเงิน หรือ รหัสร้านค้าของทางงาน ปรากฏในสลิป
