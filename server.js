...
// ข้อมูลสำหรับตรวจสอบเฉพาะผู้รับเงิน (บัญชีร้านค้าของเรา)
...
app.post('/api/verify-slip', (req, res, next) => {
    ...
    try {
        ...
        if (result && result.status === 200) {
            ...
            // --- 1. ตรวจสอบเฉพาะ "ปลายทาง/ผู้รับเงิน" ( receiver / biller / merchant ) ---
            const receiverObj = slipData.receiver || {};
            const billerObj = slipData.biller || {};
            const merchantObj = slipData.merchant || {};
            
            // แปลงข้อมูลส่วนปลายทางเป็นข้อความเพื่อค้นหา Biller ID หรือ ชื่อร้าน
            const targetText = JSON.stringify(receiverObj) + JSON.stringify(billerObj) + JSON.stringify(merchantObj);

            const isCorrectDestination = targetText.includes(TARGET_BILLER_ID) || targetText.includes(TARGET_RECEIVER_NAME);
            ...
            // บันทึกการจองเมื่อตรวจสอบผ่านทุกขั้นตอน
